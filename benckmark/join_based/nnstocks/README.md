...
**Task Table:** Nasdaq and NYSE Listed Companies (nnlist.csv)
...
**Auxiliary Table:** Wikipedia Information (nnwiki.csv)
- **Data Cleaning:**
...
- **nnlist.csv**: This file contains information on 1,078 listed companies with 11 features, covering 11 industry sector categories.

- **nnwiki.csv**: This file contains 937 retrievable Wikipedia homepage infobox records, with 22 features after cleaning.

- **mapping.csv**: This file contains the mapping relationships obtained through 1NN matching based on entity names, which can be used for join operations to leverage feature augmentation from nnwiki.csv. It has a four-column structure: (T1_index, T2_index, cosine_similarity, cosine_distance).

<p>

- **mask.pt**: This file provides the train/validation/test split for the task table (nnlist.csv). Since the dataset is class-balanced, samples within each category are randomly partitioned into train/validation/test sets at a ratio of 70%/10%/20%, respectively. The mask.pt file stores a dictionary containing three keys — train_mask, val_mask, and test_mask — each corresponding to a Boolean tensor of the same length as the total number of samples (1,078), indicating whether each sample belongs to the training, validation, or test set.
