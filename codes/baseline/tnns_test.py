...
parser.add_argument("--grid_wd", type=str, default="1e-4,5e-4,1e-3")
    ...
    if result_files:
        ...
        if not osp.exists(merged_path):
            raise FileNotFoundError(
                f"No task results matching {pattern} or merged result at {merged_path}"
            )
        print(f"Loading single-task grid result: {merged_path}")
        merged_result = load_grid_results(merged_path)
        all_results = merged_result.get("all_results", [])
        if not all_results:
            raise ValueError(f"No grid results found in {merged_path}")
