"""
Generic DA builder for unionable table pairs.

The script performs:
1. coarse schema alignment by normalized column names,
2. auxiliary-label to target-label alignment with BERT [CLS] + FAISS 1-NN,
3. row-wise appending of relabeled auxiliary samples,
4. mask extension where appended samples are training samples only.

Example:
  python da_union_generic.py \
    --task_table target.csv \
    --aux_table auxiliary.csv \
    --task_label label \
    --aux_label LABEL \
    --mask_file mask_target.pt \
    --output_dir out/union_da
"""
...
def align_labels(task_labels, aux_labels, tokenizer, model, device, batch_size, max_length):
    task_emb = compute_cls_embeddings(task_labels, tokenizer, model, device, batch_size, max_length)
    aux_emb = compute_cls_embeddings(aux_labels, tokenizer, model, device, batch_size, max_length)
...
def relabel_and_sample_aux(aux_df, aux_label, task_label, label_mapping, n_to_add, seed):
    aux = aux_df.copy()
...
def main(args):
    ...
    combined_df, aux_only_cols = append_with_unified_schema(task_df, aux_sampled, col_mapping)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generic DA builder for unionable table pairs.")
    ...
    parser.add_argument("--task_label", required=True, help="Label column in target table")
    parser.add_argument("--aux_label", required=True, help="Label column in auxiliary table")
