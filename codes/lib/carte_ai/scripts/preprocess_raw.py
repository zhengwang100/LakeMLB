...
def preprocess_data(data_name):
    ...
    if data_name == "anime_planet":
        ...
        data.drop(columns=drop_col, inplace=True)
    # LakeMLB Union datasets

    elif data_name == "maryland":
        # basic info
        target_name = "Collision_Type"
        entity_name = "Report_Number"
        task = "classification"
        repeated = False
    ...
    elif data_name == "seattle":
        # basic info
        target_name  = "COLLISIONTYPE"
        entity_name  = "OBJECTID"
        task         = "classification"
        repeated     = False
        ...
        # preprocess: drop rows where target missing
        data.dropna(subset=[target_name], inplace=True)
        data.reset_index(drop=True, inplace=True)
        data = _drop_high_null(data)
        data = _drop_single_unique(data)

        # drop unneeded / large-binary & link columns
        drop_cols = [
            ...
            "Source_of_the_collision_report",
            "Source_description"
        ]
        data.drop(columns=drop_cols, errors="ignore",inplace=True)

        # datetime columns → string categories
        ...
        cat_cols = [
            "STATUS", "ADDRTYPE", "JUNCTIONTYPE",
            "SDOT_COLCODE", "SDOT_COLDESC",
            "INATTENTIONIND", "UNDERINFL",
            "WEATHER", "ROADCOND", "LIGHTCOND",
            "HITPARKEDCAR", "SPEEDING",
            "STCOLCODE", "ST_COLDESC",
            "COLLISIONTYPE",  # ensure target stays string until encoding
        ]
        ...
        cat_cols = [
            ...
            "fuel_consumption_g_km", "offer_description", "price_in_euro"
        ]
        for col in cat_cols:
            if col in data.columns:
                data[col] = data[col].astype("str")
        ...
        # numeric columns
        num_cols = [
            "BuildingID", "RegistrationID", "BoroID", "LowHouseNumber",
            "HighHouseNumber", "StreetCode", "Postcode", "Block", "Lot",
            "CurrentStatusID", "Latitude", "Longitude", "CommunityBoard",
        ...
        # categorical columns
        cat_cols = [
            "ViolationID", "Borough", "HouseNumber", "StreetName", "Apartment",
            "Story", "Class", "NOVID", "StatuteCodes", "CurrentStatus",
            "NovType", "ViolationStatus", "RentImpairing", "NTA"
        ]
        for col in cat_cols:
            if col in data.columns:
                data[col] = data[col].astype("str")
        ...
        # basic info (note: space in column names becomes underscore in _load_raw_data)
        target_name = "VIOLATION_DESCRIPTION"
        entity_name = "ID"
        task = "classification"
        repeated = False
        ...
        # numeric columns
        num_cols = [
            "ID", "STREET_NUMBER", "INSPECTOR_ID", "INSPECTION_NUMBER",
            "PROPERTY_GROUP", "LATITUDE", "LONGITUDE"
        ]
        for col in num_cols:
            if col in data.columns:
                data[col] = pd.to_numeric(data[col], errors='coerce')
        ...
        # date columns to string
        date_cols = [
            "VIOLATION_LAST_MODIFIED_DATE", "VIOLATION_DATE",
        ...
        # categorical columns
        cat_cols = [
            "VIOLATION_STATUS", "VIOLATION_DESCRIPTION", "VIOLATION_LOCATION",
            "VIOLATION_INSPECTOR_COMMENTS", "INSPECTION_STATUS",
            "INSPECTION_WAIVED", "INSPECTION_CATEGORY", "ADDRESS",
            "STREET_DIRECTION", "STREET_NAME", "STREET_TYPE", "SSA", "LOCATION"
        ]
        for col in cat_cols:
            if col in data.columns:
                data[col] = data[col].astype("str")
    ...
    # LakeMLB Join datasets
    elif data_name == "nnstocks_stocks":
        # basic info
        target_name = "sector"
        entity_name = "symbol"
        task = "classification"
        ...
        # drop url column
        data.drop(columns=["industry"], errors="ignore", inplace=True)
        ...
        # categorical columns
        cat_cols = ["symbol", "name", "country", "url", "sector"]
        for col in cat_cols:
            if col in data.columns:
                data[col] = data[col].astype("str")
    ...
    elif data_name == "nnstocks_wiki":
        # basic info
        target_name = None # "company_type"
        ...
        # drop url columns
        drop_cols = ["wiki_url", "website"]
        data.drop(columns=drop_cols, errors="ignore", inplace=True)
        
        # categorical columns
        cat_cols = [
            "wiki_title", "company_type", "traded_as", "founded",
    ...
    elif data_name == "lh_stocks":
        # basic info
        target_name = "ICB_Industry"
        ...
        # numeric columns
        num_cols = [
            "Company_Market_Cap_(￡m)", "Stock_Code"
        ]
        for col in num_cols:
            if col in data.columns:
                data[col] = pd.to_numeric(data[col], errors='coerce')
        ...
        # date columns to string
        date_cols = [
            "Admission_Date", "Appointment_Date_(yyyy-mm-dd)", 
            "Resignation_Date_(yyyy-mm-dd)"
        ]
        for col in date_cols:
            if col in data.columns:
                data[col] = pd.to_datetime(data[col], errors='coerce').dt.strftime("%Y-%m-%d")
        ...
        # categorical columns
        cat_cols = [
            "Source", "ICB_Industry", "Country_of_Incorporation", 
            "World_Region", "Market", "International_Issuer", 
            "Listing_Status", "Director's_English_Name", "Capacity", "Position"
        ]
        for col in cat_cols:
            if col in data.columns:
                data[col] = data[col].astype("str")
    ...
    elif data_name == "lh_wiki":
        # basic info (unlabeled source table)
        target_name = None
        entity_name = "wiki_title"
        task = "classification"
        repeated = False
        ...
        # categorical columns (treat num_employees as categorical due to non-numeric values)
        cat_cols = [
            "wiki_url", "area_served", "company_type", "founded", 
            "founders", "headquarters", "industry", "key_people", 
            "num_employees", "owner", "parent", "products", 
            "subsidiaries", "traded_as", "website"
        ]
        for col in cat_cols:
            if col in data.columns:
                data[col] = data[col].astype("str")
        
        # no target encoding needed (unlabeled)

    elif data_name == "discogs":
        # basic info
        target_name = "genres"
        entity_name = "title"
        task = "classification"
        ...
        # categorical columns
        cat_cols = ["title", "artists", "genres", "region"]
        for col in cat_cols:
            if col in data.columns:
                data[col] = data[col].astype("str")
        ...
        # numeric columns (audio features)
        num_cols = [
            "Unnamed:_0", "popularity", "duration_ms", "danceability", 
        ...
        if "explicit" in data.columns:
            data["explicit"] = data["explicit"].astype(str).str.lower()
            data["explicit"] = data["explicit"].map({"true": 1, "false": 0})
            data["explicit"] = pd.to_numeric(data["explicit"], errors='coerce')
        
        # categorical columns
        cat_cols = ["track_id", "artists", "album_name", "track_name", "track_genre"]
        for col in cat_cols:
            if col in data.columns:
                data[col] = data[col].astype("str")
    ...
    #LakeMLB NNstocks ann datasets
    elif data_name == "t1_enriched_rank1":
        # basic info
        target_name = "sector"
        entity_name = "symbol"
        task = "classification"
    ...
    elif data_name == "t1_enriched_rank2":
        # basic info
        target_name = "sector"
        entity_name = "symbol"
        task = "classification"
    ...
    elif data_name == "t1_enriched_rank4":
        # basic info
        target_name = "sector"
        entity_name = "symbol"
        task = "classification"
    ...
    elif data_name == "t1_enriched_rank8":
        # basic info
        target_name = "sector"
        entity_name = "symbol"
        task = "classification"
    ...
    elif data_name == "t1_enriched_random":
        # basic info
        target_name = "sector"
        entity_name = "symbol"
        task = "classification"
    ...
    # LakeMLB MSTraffic ablation experiment tables
    elif data_name == "seattle_25pct":
        # basic info
        target_name  = "COLLISIONTYPE"
        entity_name  = "OBJECTID"
        task         = "classification"
    ...
    # LakeMLB CNBilding ablation experiment tables
    elif data_name == "chicago_25pct":
        # basic info (note: space in column names becomes underscore in _load_raw_data)
        target_name = "VIOLATION_DESCRIPTION"
        entity_name = "ID"
        task = "classification"
    ...
    # LakeMLB GACars ablation experiment tables
    elif data_name == "australian_25pct":
        # basic info
        target_name = "Price"
        entity_name = None
        task = "classification"
    ...
    elif data_name == "australian_50pct":
        # basic info
        target_name = "Price"
        entity_name = None
        task = "classification"
    ...
    elif data_name == "australian_75pct":
        # basic info
        target_name = "Price"
        entity_name = None
        task = "classification"
    ...
    #LakeMLB LHstocks 1nn datasets
    elif data_name == "stocks_lh_enriched":
        # basic info
        target_name = "ICB_Industry"
        entity_name = "Company_Name"
        task = "classification"
        ...
        # numeric columns
        num_cols = [
            "Company_Market_Cap_(￡m)", "Stock_Code", "net_income", "operating_income", "revenue", 
            "total_assets", "total_equity"
        ]
        for col in num_cols:
            if col in data.columns:
        ...
        cat_cols = [
            ...
            "World_Region", "Market", "International_Issuer", 
            "Listing_Status", "Director's_English_Name", "Capacity", "Position", 
            "wiki_title", "wiki_url", "area_served", "company_type", "founded", 
            "founders", "headquarters", "industry", "key_people", 
            "num_employees", "owner", "parent", "products", 
            "subsidiaries", "traded_as", "website"
        ]
    ...
    #LakeMLB DSMusic 1nn datasets
        ...
        # numeric columns
        num_cols = ["release_year", "Unnamed:_0", "popularity", "duration_ms", "danceability", 
            "energy", "key", "loudness", "mode", "speechiness", 
            "acousticness", "instrumentalness", "liveness", "valence", 
            "tempo", "time_signature"]
        for col in num_cols:
...
#LakeMLB Union DA
        ...
        num_cols = [
            ...
            "HighHouseNumber", "StreetCode", "Postcode", "Block", "Lot",
            "CurrentStatusID", "Latitude", "Longitude", "CommunityBoard",
            "CouncilDistrict", "CensusTract", "BIN", "BBL", "ID", "STREET_NUMBER", "INSPECTOR_ID", "INSPECTION_NUMBER",
            "PROPERTY_GROUP", 
            # "LATITUDE", "LONGITUDE"
        ]
        for col in num_cols:
        ...
        date_cols = [
            "InspectionDate", "ApprovedDate", "OriginalCertifyByDate",
            "OriginalCorrectByDate", "NewCertifyByDate", "NewCorrectByDate",
            "CertifiedDate", "NOVIssuedDate", "CurrentStatusDate", "VIOLATION_LAST_MODIFIED_DATE", "VIOLATION_DATE",
            "VIOLATION_STATUS_DATE"
        ]
        for col in date_cols:
            if col in data.columns:
        ...
        # numeric columns
        num_cols = ["year", "power_kw", "power_ps", "mileage_in_km","Kilometres"]
        for col in num_cols:
            if col in data.columns:
                data[col] = pd.to_numeric(data[col], errors='coerce')
        ...
        cat_cols = [
            ...
            "fuel_consumption_g_km", "offer_description", "price_in_euro", 
            # "Brand", "Model",
            "Car/Suv", "Title", "UsedOrNew",
            "Transmission", "Engine", "DriveType", 
            # "FuelType",
            "FuelConsumption", "ColourExtInt", "Location",
        ...
        cat_cols = [
            ...
            "World_Region", "Market", "International_Issuer", 
            "Listing_Status", "Director's_English_Name", "Capacity", "Position",
            "wiki_url", "wiki_title", "area_served", "company_type", "founded", 
            "founders", "headquarters", "industry", "key_people", 
            "num_employees", "owner", "parent", "products", 
            "subsidiaries", "traded_as", "website"
        ]
        ...
        # categorical columns
        cat_cols = ["title", "artists", "genres", "region", "track_id", "artists", "album_name", "track_name", "track_genre"]
        for col in cat_cols:
            if col in data.columns:
                data[col] = data[col].astype("str")
...
#LakeMLB Union FA
        ...
        cat_cols = [
            "ViolationID", "Borough", "HouseNumber", "StreetName", "Apartment",
            "Story", "Class", "NOVID", "StatuteCodes", "CurrentStatus",
            "NovType", "ViolationStatus", "RentImpairing", "NTA", "VIOLATION_STATUS", "VIOLATION_DESCRIPTION", "VIOLATION_LOCATION",
            "VIOLATION_INSPECTOR_COMMENTS", "INSPECTION_STATUS",
            "INSPECTION_WAIVED", "INSPECTION_CATEGORY", "ADDRESS",
            "STREET_DIRECTION", "STREET_NAME", "STREET_TYPE", "SSA", "LOCATION"
        ]
