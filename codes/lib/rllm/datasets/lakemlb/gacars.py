...
class GACarsDataset(Dataset):
    ...
    def raw_filenames(self):
        return [
            ...
            "mask_da.pt",           # 5
            "german_reg.csv",       # 6
            "australian_reg.csv",   # 7
            "gacars_da_reg.csv",    # 8 +
            "gacars_fa_reg.csv",    # 9 +
            "german_mask_reg.pt",   # 10
            "australian_mask_reg.pt", # 11
            "mask_da_reg.pt",       # 12 +
        ]
    ...
    def processed_filenames(self):
        return [
            "german_data.pt",           # 0
            "australian_data.pt",       # 1
            "gacars_da_data.pt",        # 2
            "gacars_fa_data.pt",        # 3
            "german_reg_data.pt",       # 4
            "australian_reg_data.pt",   # 5
            "gacars_da_reg_data.pt",    # 6 +
            "gacars_fa_reg_data.pt",    # 7 +
        ]
    ...
    def process(self):
        ...
        col_types = {
            ...
            "registration_date": ColType.CATEGORICAL,
            "year": ColType.NUMERICAL,
            "price_in_euro": ColType.NUMERICAL,
            "power_kw": ColType.NUMERICAL,
            "power_ps": ColType.NUMERICAL,
            "transmission_type": ColType.CATEGORICAL,
            "fuel_type": ColType.CATEGORICAL,
        ...
        german_reg_df = pd.read_csv(csv_path, low_memory=False)
        masks_path = osp.join(self.raw_dir, self.raw_filenames[10])
        masks = torch.load(masks_path, weights_only=False)
        TableData(
            df=german_reg_df,
            col_types=col_types,
        ...
        col_types = {
            ...
            "Doors": ColType.CATEGORICAL,
            "Seats": ColType.CATEGORICAL,
            "Price": ColType.NUMERICAL,
        }
        australian_reg_df = pd.read_csv(csv_path, low_memory=False)
        masks_path = osp.join(self.raw_dir, self.raw_filenames[11])
        masks = torch.load(masks_path, weights_only=False)
        TableData(
            df=australian_reg_df,
            col_types=col_types,
            target_col="Price",
            train_mask=masks["train_mask"],
            val_mask=masks["val_mask"],
        ...
        col_types = {
            ...
            "mileage_in_km": ColType.NUMERICAL,
            "offer_description": ColType.CATEGORICAL,
            # Unmatched Australian columns (NaN for German rows)
            "Car/Suv": ColType.CATEGORICAL,
            "Title": ColType.CATEGORICAL,
            "UsedOrNew": ColType.CATEGORICAL,
            "Transmission": ColType.CATEGORICAL,
        ...
        gacars_da_reg_df = pd.read_csv(csv_path, low_memory=False)
        masks_path = osp.join(self.raw_dir, self.raw_filenames[12])
        masks = torch.load(masks_path, weights_only=False)
        TableData(
            df=gacars_da_reg_df,
            col_types=col_types,
        ...
        col_types = {
            ...
            "mileage_in_km": ColType.NUMERICAL,
            "offer_description": ColType.CATEGORICAL,
            # Australian columns (1-NN matched, appended as features)
            "Brand": ColType.CATEGORICAL,
            "Year": ColType.NUMERICAL,
            "Model": ColType.CATEGORICAL,
            "Car/Suv": ColType.CATEGORICAL,
        ...
        gacars_fa_reg_df = pd.read_csv(csv_path, low_memory=False)
        masks_path = osp.join(self.raw_dir, self.raw_filenames[10])
        masks = torch.load(masks_path, weights_only=False)
        TableData(
            df=gacars_fa_reg_df,
