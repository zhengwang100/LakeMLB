...
class MSTrafficDataset(Dataset):
    ...
    def raw_filenames(self):
        return [
            ...
            "mask_da.pt",
            "seattle_25pct.csv",
            "seattle_25pct_mask.pt",
            "seattle_50pct.csv",
            "seattle_50pct_mask.pt",
            "seattle_75pct.csv",
            "seattle_75pct_mask.pt",
        ]
    ...
    def processed_filenames(self):
        return [
            ...
            "seattle_25pct_data.pt",
            "seattle_50pct_data.pt",
            "seattle_75pct_data.pt",
        ]
    ...
    def process(self):
        ...
        # Seattle labeled-data ablations used by TransTab transfer experiments.
        seattle_subset_specs = [
            (6, 7, 4),
            (8, 9, 5),
            (10, 11, 6),
        ]
        for csv_idx, mask_idx, processed_idx in seattle_subset_specs:
            subset_df = pd.read_csv(
                osp.join(self.raw_dir, self.raw_filenames[csv_idx]),
                low_memory=False,
            )
            subset_masks = torch.load(
                osp.join(self.raw_dir, self.raw_filenames[mask_idx]),
                weights_only=False,
            )
            TableData(
                df=subset_df,
                ...
                target_col="COLLISIONTYPE",
                train_mask=subset_masks["train_mask"],
                val_mask=subset_masks["val_mask"],
                test_mask=subset_masks["test_mask"],
