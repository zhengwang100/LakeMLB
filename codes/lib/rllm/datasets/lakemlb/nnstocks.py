...
class NNStocksDataset(Dataset):
    ...
    def processed_filenames(self):
        return [
            ...
            "nnstocks_da_data.pt",
            "nnstocks_fa_data.pt",
            "stocks_wiki_llm_1nn_data.pt",
            "t1_enriched_rank2_data.pt",
            "t1_enriched_rank4_data.pt",
            "t1_enriched_rank8_data.pt",
            "stocks_wiki_tfidf_1nn_data.pt",
            "t1_enriched_random_data.pt",
        ]
    ...
    def process(self):
        ...
        llm_1nn_df = pd.read_csv(csv_path, low_memory=False)
        masks_path = osp.join(self.raw_dir, self.raw_filenames[4])
        masks = torch.load(masks_path, weights_only=False)
        TableData(
            df=llm_1nn_df,
        ...
        # Rank-2 enriched data
        csv_path = osp.join(self.raw_dir, self.raw_filenames[7])
        col_types = {
            "symbol": ColType.CATEGORICAL,
            "name": ColType.CATEGORICAL,
            "lastsale": ColType.NUMERICAL,
        ...
        rank2_df = pd.read_csv(csv_path, low_memory=False)
        masks_path = osp.join(self.raw_dir, self.raw_filenames[4])
        masks = torch.load(masks_path, weights_only=False)
        TableData(
            df=rank2_df,
        ...
        # Rank-4 enriched data
        csv_path = osp.join(self.raw_dir, self.raw_filenames[8])
        col_types = {
            "symbol": ColType.CATEGORICAL,
            "name": ColType.CATEGORICAL,
            "lastsale": ColType.NUMERICAL,
        ...
        rank4_df = pd.read_csv(csv_path, low_memory=False)
        masks_path = osp.join(self.raw_dir, self.raw_filenames[4])
        masks = torch.load(masks_path, weights_only=False)
        TableData(
            df=rank4_df,
            ...
            val_mask=masks["val_mask"],
            test_mask=masks["test_mask"],
        ).save(self.processed_paths[6])

        # Rank-8 enriched data
        csv_path = osp.join(self.raw_dir, self.raw_filenames[9])
        col_types = {
            "symbol": ColType.CATEGORICAL,
            "name": ColType.CATEGORICAL,
            "lastsale": ColType.NUMERICAL,
        ...
        rank8_df = pd.read_csv(csv_path, low_memory=False)
        masks_path = osp.join(self.raw_dir, self.raw_filenames[4])
        masks = torch.load(masks_path, weights_only=False)
        TableData(
            df=rank8_df,
            ...
            val_mask=masks["val_mask"],
            test_mask=masks["test_mask"],
        ).save(self.processed_paths[7])

        # TF-IDF-joined Data (1-NN)
        csv_path = osp.join(self.raw_dir, self.raw_filenames[10])
        col_types = {
            "symbol": ColType.CATEGORICAL,
            "name": ColType.CATEGORICAL,
            "lastsale": ColType.NUMERICAL,
        ...
        tfidf_1nn_df = pd.read_csv(csv_path, low_memory=False)
        masks_path = osp.join(self.raw_dir, self.raw_filenames[4])
        masks = torch.load(masks_path, weights_only=False)
        TableData(
            df=tfidf_1nn_df,
            ...
            val_mask=masks["val_mask"],
            test_mask=masks["test_mask"],
        ).save(self.processed_paths[8])

        # Random-match enriched data
        csv_path = osp.join(self.raw_dir, self.raw_filenames[11])
        random_enriched_df = pd.read_csv(csv_path, low_memory=False)
        masks_path = osp.join(self.raw_dir, self.raw_filenames[4])
        masks = torch.load(masks_path, weights_only=False)
        TableData(
            df=random_enriched_df,
            col_types=dict(col_types),
            target_col="sector",
            train_mask=masks["train_mask"],
            val_mask=masks["val_mask"],
            test_mask=masks["test_mask"],
        ).save(self.processed_paths[9])
