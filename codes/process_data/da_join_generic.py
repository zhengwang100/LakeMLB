"""Build join-based data augmentation from row-aligned table matches.

The script transfers target labels to matched auxiliary rows, aligns compatible
columns, samples auxiliary records, and appends them as training-only examples.
Exactly one paired input is required: an enriched target table or a matched
auxiliary table.
"""
...
def extract_labeled_aux_from_enriched(enriched_df, task_df, label_col):
    ...
    cols_to_keep = [
        col for col in enriched_df.columns if col not in task_cols_to_remove
    ]
...
def main(args):
    if bool(args.enriched_table) == bool(args.matched_aux_table):
        raise ValueError(
            "Provide exactly one of --enriched_table or --matched_aux_table"
        )
    ...
    if args.enriched_table:
        ...
        labeled_aux_df = extract_labeled_aux_from_enriched(
            source_df, task_df, args.label_col
        )
        source_name = args.enriched_table
    else:
        source_df, source_encoding = read_csv_auto(
            args.matched_aux_table, args.encoding
        )
        labeled_aux_df = build_labeled_aux_from_matched(
            task_df, source_df, args.label_col
        )
    ...
    n_to_add = sample_count(
        task_df, labeled_aux_df, mask, args.sample_ratio, args.cap_basis
    )
    if n_to_add > 0:
    ...
    combined_df, aux_only_cols = append_with_unified_schema(
        task_df, sampled_aux, col_mapping
    )
    ...
    print(
        f"Pairing source: {source_name} ({source_df.shape}, encoding={source_encoding})"
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generic DA builder for joinable table pairs."
    )
    parser.add_argument("--task_table", required=True, help="Target/task table CSV")
    parser.add_argument(
        "--enriched_table",
        help="CSV containing target rows concatenated with matched aux rows",
    )
