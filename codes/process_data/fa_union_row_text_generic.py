"""Build union-based feature augmentation with row-text matching.

The script serializes each row as column-value text, retrieves the nearest
auxiliary row in BERT embedding space, and appends the matched auxiliary
features without changing the number of target rows.
"""
...
def search_1nn(index, query_embeddings):
    query_embeddings = np.ascontiguousarray(
        query_embeddings.astype("float32", copy=False)
    )
...
def print_similarity_stats(similarities, indices):
    ...
    print(
        f"  0.5 - 0.8:         {int(np.sum((matched_sims >= 0.5) & (matched_sims <= 0.8)))}"
    )
...
def main(args):
    ...
    task_text_cols = resolve_text_columns(
        task_df, task_include_cols, task_exclude_cols, "task"
    )
    aux_text_cols = resolve_text_columns(
        aux_df, aux_include_cols, aux_exclude_cols, "aux"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--batch_size", type=int, default=16, help="BERT embedding batch size"
    )
    parser.add_argument(
        "--max_length", type=int, default=512, help="Tokenizer max sequence length"
    )
    parser.add_argument("--hnsw_m", type=int, default=32, help="FAISS HNSW M parameter")
    parser.add_argument(
        "--hnsw_ef_construction", type=int, default=64, help="FAISS HNSW efConstruction"
    )
    parser.add_argument(
        "--hnsw_ef_search", type=int, default=64, help="FAISS HNSW efSearch"
    )
    parser.add_argument(
        "--threshold",
        type=float,
    ...
    parser.add_argument(
        "--output_table",
        default="augmented_table.csv",
        help="Output augmented CSV filename",
    )
    parser.add_argument(
        "--output_mapping",
        default="row_mapping.csv",
        help="Output row mapping CSV filename",
    )
    parser.add_argument(
        "--output_matched_aux",
