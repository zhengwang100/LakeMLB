...
class MSTrafficDataset(Dataset):
    ...
    def raw_filenames(self):
        return ["maryland.csv", "maryland_mask.pt"]

    @property
    def processed_filenames(self):
        return ["mstraffic_maryland_data_1.pt"]
    ...
    def download(self):
        ...
        # If url is not defined, check if files exist manually or raise error
        if not hasattr(self, 'url') or self.url is None:
            # Check if required files exist
            required_files = self.raw_filenames
            missing_files = [f for f in required_files if not osp.exists(osp.join(self.raw_dir, f))]
            if missing_files:
                raise FileNotFoundError(
                    f"Required data files not found: {missing_files}\n"
                    f"Please ensure these files exist in: {self.raw_dir}\n"
                    f"Or uncomment the 'url' attribute in MSTrafficDataset to enable automatic download."
                )
            return
        
        # Download from URL if url is available
