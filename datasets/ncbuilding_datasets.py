...
class NCBuildingDataset(Dataset):
    ...
    def raw_filenames(self):
        return ["newyork.csv", "mask_newyork.pt"]

    @property
    def processed_filenames(self):
        return ["ncbuilding_newyork_data.pt"]

    def process(self):
        os.makedirs(self.processed_dir, exist_ok=True)
        parquet_path = osp.join(self.raw_dir, self.raw_filenames[0])
        masks_path = osp.join(self.raw_dir, self.raw_filenames[1])
        df = None
        for encoding in ['utf-8', 'utf-8-sig', 'gbk', 'gb18030', 'latin1']:
            try:
                df = pd.read_csv(parquet_path, encoding=encoding)
