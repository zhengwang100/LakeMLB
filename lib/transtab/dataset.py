    ...
    if os.path.isdir(dataname):
        ...
        if dataset_config is not None:
            if 'columns' in dataset_config:
                ...
                if X.columns.duplicated().any():
                    dup_cols = X.columns[X.columns.duplicated()].tolist()
                    logger.warning(f"Duplicated columns after renaming, keep first: {dup_cols}")
                    X = X.loc[:, ~X.columns.duplicated()].copy()
        ...
        if dataset_config is not None:
            ...
            if 'bin' in dataset_config:
                bin_cols = [c for c in bin_cols if c in cols_in_data]
            if 'cat' in dataset_config:
            ...
            def _unique_keep_order(cols):
                ...
                return result
            if 'bin' in dataset_config:
                bin_cols = _unique_keep_order(bin_cols)
            if 'cat' in dataset_config:
                cat_cols = _unique_keep_order(cat_cols)
            if 'num' in dataset_config:
                num_cols = _unique_keep_order(num_cols)
