...
class SupervisedTrainCollator(TrainCollator):
    ...
    def __call__(self, data):
        x = pd.concat([row[0] for row in data], ignore_index=True)
        
        # Handle y values: must be all None or all not None
...
class TransTabCollatorForCL(TrainCollator):
    ...
    def __call__(self, data):
        ...
        Parameters
        ----------
        data : list of tuples (df_x, df_y)
            df_y can be None for self-supervised/contrastive learning.
        '''
        # 1. Concatenate features from all samples
        df_x = pd.concat([row[0] for row in data], ignore_index=True)
        
        # 2. Handle labels: must be all None or all not None
        y_list = [row[1] for row in data]
        if all(y is None for y in y_list):
            # Self-supervised learning: no labels
            df_y = None
        else:
            # Supervised learning: concatenate all labels
            df_y = pd.concat(y_list, ignore_index=True)
        
        # 3. Build positive pairs for contrastive learning
        ...
        # 4. Encode each sub-view with feature extractor
        input_x_list = []
